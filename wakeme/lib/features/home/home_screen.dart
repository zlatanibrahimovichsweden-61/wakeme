...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  Future<void> _bootstrap() async {
    ...
    // Battery-optimization exemption improves alarm reliability while the phone
    // is idle. Asked ONCE, non-blocking — the user can decline and WakeMe still
    // runs. No "Allow all the time" prompt: the foreground service started on
    // Sleep keeps tracking while locked/backgrounded WITHOUT the
    // ACCESS_BACKGROUND_LOCATION permission (so we avoid Play's background-
    // location review entirely).
    ...
    final bool? confirm = await showDialog<bool>(
      ...
      builder: (BuildContext ctx) {
        return AlertDialog(
          ...
          content: Text(
            'WakeMe will replace the saved coordinates of "${place.name}" with where you are right now.',
